...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        preact = self.residual_function(x) + self.shortcut(x)
        return nn.ReLU(inplace=True)(preact), preact
...
class BottleNeck(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.is_last:
            ...
            return nn.ReLU(inplace=True)(preact)
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, out_channels, num_blocks, stride):
        ...
        for idx, stride in enumerate(strides):
            layers.append(block(self.base_channels, out_channels, stride, is_last=(idx==num_blocks-1)))
            self.base_channels = out_channels * block.expansion
